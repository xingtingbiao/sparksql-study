...
object TopNStatJob2 {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val commonDF = frame.filter($"day" === day && $"cmsType" === "video")
    // 缓存复用的数据
    commonDF.cache()
    ...
    // 按流量flow统计最受欢迎的课程
    flowAccessTopNStat(spark, commonDF)
    // 释放缓存的数据
...
//    val videoAccess = spark.sql("select day, cmsId, count(1) as times from video_access where day = '20170511' and cmsType = 'video' group by day, cmsId order by times desc")
  ...
  /**
    * 按流量统计的结果并插入到数据库
    */
  def flowAccessTopNStat(spark: SparkSession, commonDF: DataFrame): Unit = {
    import spark.implicits._
    val flowDF = commonDF
      .groupBy("day", "cmsId")
